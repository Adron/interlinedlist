      ...
      case 'users':
        [data, total] = await Promise.all([
          prisma.user.findMany({
            ...
            // SECURITY: explicit allowlist. Never return password hashes, auth
            // tokens, or stored third-party API keys to the client — not even for
            // the admin data browser. New columns must be added here deliberately.
            select: {
              id: true,
              email: true,
              username: true,
              ...
              // EXCLUDED: passwordHash, passwordResetToken, emailVerificationToken,
              // emailChangeToken, openaiApiKey, anthropicApiKey
            },
          }),
          prisma.user.count(),
        ]);
